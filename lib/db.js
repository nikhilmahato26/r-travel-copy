...
import ws from 'ws'
import { ALL_PACKAGES as SEED_PACKAGES } from './packages-data.js'
...
export async function togglePackageFeatured(id, featured, order = 0) {
  ...
  await pool.query(
    `UPDATE packages SET featured = $1, featured_order = $2,
       featured_at = CASE WHEN $1 THEN COALESCE(featured_at, NOW()) ELSE NULL END,
       updated_at  = NOW()
     WHERE id = $3`,
    [featured, order, id]
  )
...
export async function initDestinationsTable() {
  ...
  await pool.query(`
    INSERT INTO destinations (name, color, image_url, description, emoji) VALUES
      ...
      ('Gujarat',       '#f59e0b', 'https://images.unsplash.com/photo-1590050752117-238cb0612b1b?w=800&q=80', 'Land of legends, lions, and white sands', '🛕'),
      ('Uttar Pradesh', '#8b5cf6', 'https://images.unsplash.com/photo-1561361513-2d000a50f0dc?w=800&q=80', 'Heartland of culture, heritage, and spirituality', '🕌'),
      ('Uttarakhand',   '#10b981', 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?w=800&q=80', 'Devbhoomi - the land of gods and mountains', '🏔️'),
      ('Himachal',      '#06b6d4', 'https://images.unsplash.com/photo-1544735716-392fe2489ffa?w=800&q=80', 'Snow-capped peaks and scenic valley highways', '🏔️'),
      ('Jammu Kashmir', '#3b82f6', 'https://images.unsplash.com/photo-1566228015668-4c45dbc4e2f5?w=800&q=80', 'Paradise on Earth - lakes, gardens, and snow', '❄️'),
      ('North East',    '#047857', 'https://images.unsplash.com/photo-1548013146-72479768bada?w=800&q=80', 'Unexplored green hills and scenic monasteries', '🎋'),
      ('Bengal & Odisha','#14b8a6', 'https://images.unsplash.com/photo-1588610738110-ee976bc940e2?w=800&q=80', 'Culture, temples, and coastal delta charms', '🌊'),
      ('Kerala',        '#16a34a', 'https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=800&q=80', 'God''s Own Country - backwaters and hills', '🌴'),
      ('Tamil Nadu',    '#b91c1c', 'https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=800&q=80', 'Land of monumental temples and rich history', '🛕'),
      ('Goa',           '#0ea5e9', 'https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=800&q=80', 'Sun-kissed beaches, parties, and heritage', '🏖️'),
      ('Maharashtra',   '#f97316', 'https://images.unsplash.com/photo-1562158074-a285d898516d?w=800&q=80', 'Gateway of India, caves, and mountain fortresses', '🏰'),
      ('Karnataka',     '#eab308', 'https://images.unsplash.com/photo-1600100397990-24b3500c8b09?w=800&q=80', 'Palaces, ancient Hampi ruins, and coffee hills', '🏛️')
...
export async function initSettingsTable() {
  ...
  `)
  await pool.query(`INSERT INTO settings (key, value) VALUES ('phone', '919427286755') ON CONFLICT DO NOTHING`)
  await pool.query(`INSERT INTO settings (key, value) VALUES ('whatsapp', '919427286755') ON CONFLICT DO NOTHING`)
...
export async function initEnquiriesTable() {
  ...
  `)
  await pool.query(`ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS destination TEXT`)
  await pool.query(`ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS type TEXT NOT NULL DEFAULT 'package'`)
}

export async function createEnquiry({ package_id, package_title, destination, type, name, phone, email, message }) {
  const pool = getPool()
  await initEnquiriesTable()
  const t = ['package', 'flight', 'train'].includes(type) ? type : 'package'
  const { rows } = await pool.query(
    'INSERT INTO enquiries (package_id, package_title, destination, type, name, phone, email, message) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
    [package_id || null, package_title || null, destination || null, t, name, phone, email || null, message || null]
  )
  return rows[0]
}
